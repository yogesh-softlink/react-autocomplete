...
// Remove the problematic findDOMNode import
...
// Helper function to get DOM node from ref
function getDOMNode(ref) {
  ...
  // If it's a React component instance with a DOM node
  if (ref.current && ref.current.nodeType) return ref.current;
  
  // If it's a React component with getDOMNode method (legacy)
  if (ref.getDOMNode && typeof ref.getDOMNode === 'function') {
    return ref.getDOMNode();
  }
  
  // If it's a React component, try to get the DOM node
  if (ref._reactInternalFiber || ref._reactInternalInstance) {
    // For newer React versions, try to access the DOM node directly
    return ref;
  }
  ...
  _createClass(Autocomplete, [{
    ...
    value: function maybeScrollItemIntoView() {
      if (this.isOpen() && this.state.highlightedIndex !== null) {
        ...
        // Use the custom getDOMNode function instead of findDOMNode
        var itemDOMNode = getDOMNode(itemNode);
        var menuDOMNode = getDOMNode(menuNode);
        
        if (itemDOMNode && menuDOMNode) {
          scrollIntoView(itemDOMNode, menuDOMNode, {
    ...
    value: function render() {
      ...
      }, this.props.wrapperProps), this.props.renderInput(_objectSpread(_objectSpread({}, inputProps), {}, {
        role: 'combobox',
      ...
      })), open && this.renderMenu(), this.props.debug && /*#__PURE__*/React.createElement("pre", {
        style: {
          marginLeft: 300
        }
